...
from analytics.models import TaskInstance, WorkflowInstance, Workflow, User, Role, Document, Department, BottleneckScoreWeights, Permission
...
# reused methods
    ...
    # TASK INSTANCES TABLE (raw list for the Overall tab, next to the SLA
    # distribution pie chart) — every task_instance column the SLA engine
    # reads/writes, filtered by created_at (unlike completed_tasks() above,
    # this needs to include not-yet-completed tasks too, so status/sla_status
    # actually show variety instead of always reading "completed").
    @staticmethod
    def task_instances(date_from=None, date_to=None):
    ...
    def role_permissions():
        """
        What each role is authorized to do (Approve/Review/Create/Delete),
        from analytics_role_permission. Read with plain SQL rather than the
        ORM since that table is a composite-key junction with no id column.
        """

        roles = {r.role_id: r.role_name for r in Role.objects.all()}
        permissions = {p.permission_id: p.permission_name for p in Permission.objects.all()}

        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT role_id, permission_id FROM analytics_role_permission "
        ...
        for role_id, permission_id in links:
            entry = by_role.setdefault(role_id, {
                "role_id": role_id,
                "role_name": roles.get(role_id, f"Role {role_id}"),
                "permissions": [],
            })
            entry["permissions"].append(permissions.get(permission_id, f"Permission {permission_id}"))

        return sorted(by_role.values(), key=lambda r: r["role_id"])