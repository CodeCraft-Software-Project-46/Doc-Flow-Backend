...
# analytics_role_permission is a plain (role_id, permission_id) junction
# table with a composite primary key and no surrogate id column, so it's
# read directly with SQL in OverallWidgets.role_permissions() rather than
# modeled here — Django's unmanaged-model ORM mapping assumes one field
# carries object identity, which doesn't fit a table with no single-column
# key.
...
class Document(models.Model):
    ...
    document_summary = models.CharField(max_length=255, null=True, blank=True)
